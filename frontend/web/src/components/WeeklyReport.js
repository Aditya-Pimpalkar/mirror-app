  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, padding: "40px 28px", textAlign: "center" }}>
        ...
        <div style={{ fontFamily: "var(--font-body)", color: "var(--text-muted)", fontStyle: "italic", fontSize: 13 }}>
          Synthesizing this week's conversations...
        </div>
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "48px 24px 100px" }}>
      ...
      <div className="card" style={{ marginBottom: 16, textAlign: "center" }}>
        ...
        {report?.gapDelta !== undefined && report.gapDelta !== 0 && (
          <div style={{ marginTop: 12, fontFamily: "var(--font-mono)", fontSize: 11, color: report.gapDelta < 0 ? "#7DC98F" : "#E87070" }}>
            {report.gapDelta < 0 ? "▼" : "▲"} {Math.abs(report.gapDelta)} from last week
          </div>
        )}
      </div>
          ...
          {Object.values(PERSONAS).map(p => (
            <div key={p.id}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                ...
                <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--text-muted)" }}>{beliefs[p.id] || p.initialBelief}/100</span>
              </div>
      ...
      {report?.content && (
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <p style={{ fontFamily: "var(--font-body)", color: "var(--text-muted)", fontSize: 14, lineHeight: 1.7, fontStyle: "italic", margin: 0 }}>
            {typeof report.content === "string" ? report.content : report.content?.summary || "No analysis available."}
          </p>
        </div>
      )}
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-around", textAlign: "center" }}>
          <div>
            ...
            <div className="label" style={{ fontSize: 9 }}>PERSONAS TALKED TO</div>
          </div>
          <div>
